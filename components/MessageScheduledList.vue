                ...
                <div class="d-inline-block d-md-none">
                  <div class="dropdown">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdown-message-options">
                      ...
                      <li><a href="/messages/scheduled" class="dropdown-item" :class="{'disabled': selected.length === 0}" @click.prevent="unqueueBatch" v-if="hasRole('admin') || can('message-unqueue')">{{ $t('unqueue') }}</a></li>
                      ...
                      <tr :class="{'table-active': selected.includes(message.id) }">
                        ...
                        <td class="text-end">
                          <div class="btn-group btn-group-sm">
                            ...
                            <button type="button" class="btn btn-light" :title="$t('unqueue')" @click.prevent="unqueue(message)" v-if="hasRole('admin') || can('message-unqueue')"><i class="mdi mdi-email-minus-outline"></i></button>
...
const unqueue = async (message: IMessage): Promise<void> => {
  const model = t('the_message')
  const name = message.name
  const confirmMessage = t('messages.confirm_message_unqueue_name', { name })
  if (confirm(confirmMessage)) {
    await useApi(`/admin/messages/unqueue/${message.id}`, {
      method: 'post',
      onResponse({ response }) {
...
const unqueueBatch = async (): Promise<void> => {
  let models = t('the_messages').toLowerCase()
  let message = t('messages.confirm_unqueue_batch_models', { models })
  if (confirm(message)) {
    await useApi(`/admin/messages/unqueue-batch`, {
      method: 'post',
      body: {
        ids: selected.value
      },
      onResponse({ response }) {        
        if (response?._data?.count) {
          ...
          addToast({
            header: t('success'),
            body: t('messages.models_unqueued', { models })
          })
        }
      },
    })
